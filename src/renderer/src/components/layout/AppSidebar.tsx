                            ...
                            <SidebarMenuItem>
                              <SidebarMenuButton
                                ...
                                tooltip={`${session.title}\n${session.mode} · ${(() => {
                                  const m = Math.floor((Date.now() - session.updatedAt) / 60000)
                                  if (m < 1) return 'just now'
                                  if (m < 60) return `${m}m ago`
                                  const h = Math.floor(m / 60)
                                  if (h < 24) return `${h}h ago`
                                  return `${Math.floor(h / 24)}d ago`
                                })()} · ${session.messageCount} msgs${session.pinned ? ' · pinned' : ''}`}
                              >
                                {session.pinned ? (
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-2">
                <p>{t('sidebar.deleteConfirm', { title: deleteTarget?.title })}</p>
